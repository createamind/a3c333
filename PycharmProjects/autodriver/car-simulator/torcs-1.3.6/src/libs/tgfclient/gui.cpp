...
//#include "../iceapi/utils/msp_log.h"
...
//	MSP_DEBUG("key %d pressed", (int)key);
	/* user preempt key */
...
gfuiSpecial(int key, int /* x */, int /* y */)
{
	...
	modifier = glutGetModifiers();
//	MSP_DEBUG("key special %d pressed", (int)key);
	/* user preempt key */
...
gfuiKeyboardUp(unsigned char key, int /* x */, int /* y */)
{
	...
	modifier = glutGetModifiers();
//	MSP_DEBUG("key up %d pressed", (int)key);
	/* user preempt key */
...
gfuiSpecialUp(int key, int /* x */, int /* y */)
{
	...
	modifier = glutGetModifiers();
//	MSP_DEBUG("key special up %d pressed", (int)key);
	/* user preempt key */
...
#include <unistd.h>
#include <string>
#include "../iceapi/utils/utils.h"
void gfuiPressKey(const std::string & keyname, int duration) {
    ...
    for (int i = 0;i < ARRAYSIZE(keymaps); i++) {
        if (keyname == keymaps[i].name) {
            key = keymaps[i].key;
            if (keymaps[i].isSpecial) {
                gfuiSpecial(key, 0, 0);
                if (duration > 0) {
                    usleep(1000 * duration);
                    gfuiSpecialUp(key, 0, 0);
                }
            } else {
                gfuiKeyboard(key, 0, 0);
                if (duration > 0) {
                    usleep(1000 * duration);
                    gfuiKeyboardUp(key, 0, 0);
                }
            }
//			MSP_INFO("press key %s", keyname.c_str());
            return;
        }
    }
//    MSP_ERROR("press key failed: can not find key define %s", keyname.c_str());
}
...
#include <vector>
void GfuiCaptureScreen(int & width, int & height, std::vector<uint8_t> & image) {
	int sw, sh;
	GfScrGetSize(&sw, &sh, &width, &height);
	image.resize(width*height*3);
	glPixelStorei(GL_PACK_ROW_LENGTH, 0);
	glPixelStorei(GL_PACK_ALIGNMENT, 1);
	glReadBuffer(GL_FRONT);
	glReadPixels((sw-width)/2, (sh-height)/2, width, height, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*)(&image[0]));
}
