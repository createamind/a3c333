...
//            oppSensorOut[i] *= normRand(1,__OPP_NOISE_STD__);
    ...
    if (m_raceInitParams->screenCapture) {
        auto image = raceStatus->image = std::make_shared<Race::Image>();
        GfuiCaptureScreen(image->width, image->height, image->data);
    }
    if (internalError) {
        raceStatus->speedX = 0.;
        raceStatus->angle = 0.;
