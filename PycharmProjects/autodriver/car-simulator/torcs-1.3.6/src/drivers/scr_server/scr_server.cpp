...
#include <memory>
#include "utils/ice_server.h"
#include "utils/race_server.h"
#include "utils/race_bot_scr.h"
#include "utils/msp_log.h"
MSP_MODULE_DECLARE("scr_server");
...
/*** defines for UDP *****/
#define UDP_LISTEN_PORT getUDPListenPort()
#define UDP_ID "SCR"
...
initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
    ...
    *carParmHandle = NULL;
    ...
    auto iceEnv = getIceEnv();
    auto bot = iceEnv->getRaceServer()->getBot(index);
    if (!bot) {
        MSP_INFO("init scr bot %d", index);
        bot = std::make_shared<RaceBot_SCR>(*iceEnv->getRaceServer(), index);
        iceEnv->getRaceServer()->setBot(index, bot);
    }
    RaceBot_SCR * scr = dynamic_cast<RaceBot_SCR*>(bot.get());
    assert(scr);
    if (scr) {
        scr->newTrack(track, carHandle, carParmHandle, s);
    }
}
...
newrace(int index, tCarElt* car, tSituation *s)
{
    ...
    if (iceEnv && iceEnv->getRaceServer()) {
        iceEnv->getRaceServer()->getBot(index)->newRace(car, s);
        MSP_NOTICE("scr bot %d new race", index);
        return;
    }
#if 0
    total_tics[index]=0;
...
drive(int index, tCarElt* car, tSituation *s)
{
    auto iceEnv = getIceEnv();
    if (iceEnv && iceEnv->getRaceServer()) {
        iceEnv->getRaceServer()->getBot(index)->drive(car, s);
        return;
    }
#if 0
  /////////////////////////////////////////////////////////////////////////////////////////////////////////
...
endrace(int index, tCarElt *car, tSituation *s)
{
    auto iceEnv = getIceEnv();
    if (iceEnv && iceEnv->getRaceServer()) {
        iceEnv->getRaceServer()->getBot(index)->endRace(car, s);
        MSP_NOTICE("scr bot %d end race", index);
        return;
    }
#if 0
    RESTARTING[index]=0;
...
shutdown(int index)
{
    ...
    auto iceEnv = getIceEnv();
    if(iceEnv && iceEnv->getRaceServer()) {
        MSP_NOTICE("scr bot %d shutdown", index);
        iceEnv->getRaceServer()->setBot(index, nullptr);
        return;
    }
#if 0
#ifdef __PRINT_RACE_RESULTS__
