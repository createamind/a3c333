...
from tensorpack.graph_builder.input_source import StagingInputWrapper, QueueInput
...
class MultiGPUTrainer(SyncMultiGPUTrainerParameterServer):
    ...
    def _setup(self):
        ...
        for n, op in model._thread_ops.items():
            auxTrainOp = _AuxTrainOp('thread-' + n)
            assert(len(op._ops) > 0)
            auxTrainOp._train_op = tf.group(*op._ops, name=n + '/thread-op')
            train_ops_aux[n] = auxTrainOp
